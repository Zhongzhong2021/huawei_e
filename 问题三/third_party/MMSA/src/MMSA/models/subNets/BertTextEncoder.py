...
class BertTextEncoder(nn.Module):
    def __init__(self, use_finetune=False, transformers='bert', pretrained='bert-base-uncased'):
        ...
        if not (source / 'config.json').is_file():
            raise FileNotFoundError(f'local BERT config missing: {source / "config.json"}')
        self.tokenizer = None  # This adapter receives token IDs from the aligned input.
        if any((source / name).is_file() for name in ('model.safetensors', 'pytorch_model.bin')):
            self.model = model_class.from_pretrained(str(source), local_files_only=True)
        else:
            self.model = model_class(BertConfig.from_json_file(str(source / 'config.json')))
