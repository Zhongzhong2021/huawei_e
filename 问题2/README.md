...
- `tests/`：53项自动测试，覆盖数据、标签、掩码、缺失信息隔离、空输入、词表、Bootstrap配对、加权梯度累积等价性、延后类别权重、检查点故障保护、固定概率融合配对、重训预检查与新旧轮次入口路由。
...
第五轮已按“中断后关闭”归档，正式模型仍为第四轮。延后1轮加权完成三折但未通过条件，延后2轮没有完整结果；未延长旧截止时间，也没有新增官方验证或测试评价。参见[第五轮分析与恢复核验](docs/检查点保护与第五轮关闭说明.md)。

训练器现在先把参数复制至CPU，再写同目录临时文件，落盘后逐张量重载校验，最后原子替换 `best.pt`。保存阶段写入 `checkpoint_events.jsonl`，异常临时文件保留供排查。3次真实12层模型保存与一次原配方内部折重跑均已核验，217个参数张量完全一致，选择分数差为0。CPU存储标记与保存容器不同会改变新检查点的文件哈希，不应据此误判参数变化。
